...
int main()
{
	...
	app.Setup([&]()
	{
		...
		tank.AddComponent<cedar::SpriteComponent>(assetManager.GetTexture("tank-panther-right"), 32, 32);

		auto truck = app.Manager()->CreateEntity();
		truck.AddComponent<cedar::RigidBodyComponent>(glm::vec2(30.f, -10.f));
		truck.AddComponent<cedar::SpriteComponent>(assetManager.GetTexture("truck-ford-right"), 32, 32);
